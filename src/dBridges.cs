...
namespace dBridges
{
    public class dBridges
    {
        ...
        public dBridges()
        {
            ...
            this.channel = new channel.station(this);
            this.options = null;
        ...
        public void access_token(Delegate callback)
        {
            if (callback == null) throw (new dBError("E004"));
        ...
        public async Task accesstoken_dispatcher(string channelName, string action, object response)
        {

            if (this.dispatch.isExists("dbridges:access_token")){

                await this.dispatch.emit2("dbridges:access_token" , channelName ,  this.sessionid , action ,  response );
                }else{
                throw (new dBError("E004"));
                }
        }
        ...
        public async Task connect()
        {
            ...
            CApiResponce result = await this.GetDBRInfo(this.auth_url, this.appkey);
            ...
            this.ClientSocket.On("db",  (response) =>
            {
            ...
            this.ClientSocket.OnDisconnected += (sender, e) =>
            {
                var task = Task.Run( () =>  this.IODisconnect(e as string));
            };
            ...
            try
            {
                await this.ClientSocket.ConnectAsync();
            }catch(Exception)
            {
        ...
        private async Task IOMessage(dbMessage dbmessage)
        {
            ...
            switch(dbmessage.dbType)
            {
                ...
                case MessageType.CONNECT_TO_CHANNEL:
                   await this.Handle_CONNECT_MSG(dbmessage.subject, dbmessage.payload, dbmessage);
                    break;
                ...
                case MessageType.RPC_RESPONSE_TRACKER:
                    ...
                    if (rpccs.GetType().Equals(typeof(CrpCaller)))
                    {
                ...
                case MessageType.RPC_CALLEE_QUEUE_EXCEEDED:
                    ...
                    if (rpccsn.GetType().Equals(typeof(CrpCaller)))
                    {
        ...
        private async Task reconnect()
        {
            if (this.retryCount >= this.maxReconnectionRetries)
            {
                try
                {
                    ...
                    //if (this.ClientSocket != null) this.ClientSocket. = null;
        ...
        return Convert.ToInt32(Math.Round(delay));
    }
        ...
        private async Task Handle_SYSTEM_MSG(string subject , string payload, long t1 , dbMessage dbmessage)
        {
            ...
            switch (subject)
            {
                case "connection:success":
                    ...
                    int milliseconds = (this.minUptime <= 0) ? 200 : (int) this.minUptime;
    ...
    public bool isSocketConnected()
    {
        ...
        private async Task Handle_CONNECT_MSG(string subject, string payload, dbMessage dbmessage)
        {
            ...
            switch (subject)
            {
                case "success":
                    switch (subscribe_status)
                    {
                        case channelState.CONNECTION_INITIATED:
                            await this.channel.updateChannelsStatusAddChange(0, dbmessage.sid, channelState.CONNECTION_ACCEPTED, "");
                            break;

                        case channelState.CONNECTION_ACCEPTED:
                        case channelState.CONNECTION_PENDING:
                            await this.channel.updateChannelsStatusAddChange(1, dbmessage.sid, channelState.CONNECTION_ACCEPTED, "");
                            break;
                        default:
                ...
                default:
                    ...
                    {
                        case channelState.CONNECTION_INITIATED:
                            await this.channel.updateChannelsStatusAddChange(0, dbmessage.sid, channelState.CONNECTION_ERROR, dberr);
                            break;

                        case channelState.CONNECTION_ACCEPTED:
                        case channelState.CONNECTION_PENDING:
                            await this.channel.updateChannelsStatusAddChange(1, dbmessage.sid, channelState.CONNECTION_PENDING, dberr);
                            break;
                    }
                    break;
            }
        ...
        private async Task Handle_PUBLISH_MSG(string subject, string payload, dbMessage dbmessage)
        {
            ...
            metadata.channelName = (mchannelName.ToLower().StartsWith("sys:*")) ? dbmessage.fenceid : mchannelName;
                ...
                await this.channel.handledispatcherEvents_publish(subject, payload, mchannelName, metadata);
        }
        ...
        private dBExtraData ConvertExtraData(string sourceip, string sourceid, string channelname ="")
        {
            ...
            if (!string.IsNullOrEmpty(sourceid))
            {
                ...
                if (strData.Length > 1) extraData.sessionid = strData[0];
                if (strData.Length > 2) extraData.libtype = strData[1];
                if (strData.Length > 3) extraData.sourceipv4 = strData[2];
                if (strData.Length > 4) extraData.sourceipv6 = strData[3];
                if (strData.Length >= 5)
                {
