...
namespace dBridges.exceptions
{
    public class dBError : Exception
    {
        ...
        public dBError(string ekey, string codeext = "", string message = "") : base(message)
        {
            ...
            {
                this.updateClassProperty(ekey, ekey, ekey, "key is missing from the lookup tables. contact support");
                return;
            }
            ...
            if(value.Length != 2)
            {
                this.updateClassProperty(ekey, ekey, ekey, "key , value structure is invalid. contact support");
                return;
            }


            if (!sourceMessage.sourceLookup.ContainsKey(value[0])){
                this.updateClassProperty(ekey, value[0].ToString(), "", "source lookup key is invalid. contact support");
                return;
            }

            if (!codeMessage.codeLookup.ContainsKey(value[1]))
            {
                this.updateClassProperty(ekey, sourceMessage.sourceLookup[value[0]] , value[1].ToString(), "code lookup key is invalid. contact support");
                return;
            }
